...
4.1 The process SHALL emit `postcore_integrity_index.json`.  
...
5.1 `postcore_integrity_index.json` SHALL be deterministic JSON.  
...
- `integrity_index_version` (string)  
- `publish_bundle_archive_hash` (string)  
- `artifacts` (array)

5.6 `integrity_index_version` SHALL equal "v1.1".  
5.7 `publish_bundle_archive_hash` SHALL equal the SHA-256 hash from `publish_bundle_manifest.json`.  
5.8 `artifacts` SHALL be lexicographically sorted by `artifact_id`.

## 6. Artifact Entry Object

6.1 Each entry object in `artifacts` SHALL contain:
...
7.1 For identical inputs, the integrity index SHALL be identical.  
...
9.2 On failure, no integrity index SHALL remain.  
